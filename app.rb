...
def handle_location(event, user_id, lat, lng)
  ...
  Position.create(user_id: user_id, lat: lat, lng: lng)
...
def handle_message(event, user_id, is_vip, group_id)
  ...
  when Line::Bot::Event::MessageType::Location
    group_id ? handle_location(event, user_id, event.message['latitude'], event.message['longitude']) : reply_text(event, '請於群組中使用')

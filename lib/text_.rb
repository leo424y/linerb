def handle_text event, user_id, group_id, origin_message
  ...
  name = clean_message.chomp('嗎').chomp('有沒有開').chomp('開了沒').chomp('有開').chomp('開了').chomp('は開いていますか').chomp('現在').chomp('附近').chomp('推薦').chomp('放口袋').chomp('里長').chomp('揪團').chomp('+1')
  # wiki_text = wiki_content event, name

  if origin_message.end_with?('里長', '附近', '揪團', '推薦有開嗎', '放口袋', '？！', '+1')

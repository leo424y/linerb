def handle_text_basic event, user_id, group_id, name, origin_message
    ...
    begin
      if name.end_with? '運動中心'
        ...
        if (!r[:open_now].to_s.empty? && (r[:open_now].to_s == 'true' || r[:open_now].to_s == 'false'))
          ...
          reply_join_vip_info(event, name, user_id, group_id, opening_hour_info) if user_id && group_id && !(is_vip user_id)
  ...
  reply_content event, message_buttons_h(name, message_buttons_text, (handle_button place_id, name, s_link, group_id, user_id), message_buttons_text)
end
